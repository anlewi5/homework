...
class MedusaTest < Minitest::Test
  ...
  def test_can_only_have_three_victims
    medusa = Medusa.new("Cassiopeia")
    victim1 = Person.new("StupidHero1")
    victim2 = Person.new("StupidHero2")
    victim3 = Person.new("StupidHero3")
    victim4 = Person.new("StupidHero4")
    ...
    stoned_count = 0
    medusa.statues.each {|victim|
      stoned_count += 1 if victim.stoned? == true}
    assert_equal 3, stoned_count
  end
  ...
  def test_if_a_fourth_victim_is_stoned_first_is_unstoned
    ...
    refute medusa.statues.first.stoned?
  end
